const API_BASE_URL =
  "https://port-0-server-m7tucm4sab201860.sel4.cloudtype.app";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function updateDateDisplay() {
    ...
    if (summaryDateEl) {
      summaryDateEl.textContent = `${currentDate.getFullYear()}년 ${
        currentDate.getMonth() + 1
      }월 ${currentDate.getDate()}일 센서별 평균값`;
    }
  ...
  sensorTabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      document
        .getElementById(`${sensorId}-sensor-chart`)
  ...
  captureButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const hwResponse = await fetch(
        `https://api.hotpotato.me/get-image?farmId=${farmId}`,
        {
          method: "GET",
        }
      );
  ...
  async function fetchAllImages(farmId) {
    ...
    try {
      ...
      galleryEmpty.querySelector(".empty-state h3").textContent =
        "이미지 로드 실패";
  ...
  function renderImages() {
    ...
    const sortedImages = [...allImages].sort((a, b) => {
      const dateA = new Date(
        a.uploadedAt
          ? `${a.uploadedAt.year}-${a.uploadedAt.month}-${a.uploadedAt.day}`
          : 0
      );
      const dateB = new Date(
        b.uploadedAt
          ? `${b.uploadedAt.year}-${b.uploadedAt.month}-${b.uploadedAt.day}`
          : 0
      );
    ...
    sortedImages.forEach((image, index) => {
      ...
      const dateStr = uploadedAt
        ? `${uploadedAt.year}-${String(uploadedAt.month).padStart(
            2,
            "0"
          )}-${String(uploadedAt.day).padStart(2, "0")}`
        : "날짜 정보 없음";

      const timeStr =
        uploadedAt &&
        uploadedAt.hour !== undefined &&
        uploadedAt.minute !== undefined
          ? `${String(uploadedAt.hour).padStart(2, "0")}:${String(
              uploadedAt.minute
            ).padStart(2, "0")}`
          : "시간 정보 없음";
  ...
  function fetchFarmStatus() {
    fetch(`${API_BASE_URL}/get-farm-status/${farmId}`)
      ...
      .then((data) => {
        const { farmname, growthRate, harvestDays, startDate, farmActive } =
          data;
        if (farmActive === 1) {
          if (startButton) startButton.style.display = "none";
          if (cropInfo) cropInfo.classList.add("visible");
  ...
  function updateGrowthStatus(growthRate, harvestDays, startDate) {
    ...
    if (growthCircle) {
      growthCircle.style.background = `conic-gradient(#ffffff 0deg ${
        growthRate * 3.6
      }deg, rgba(255,255,255,0.3) ${growthRate * 3.6}deg 360deg)`;
    }
  ...
  async function fetchFarmOptimalValues() {
    try {
      const response = await fetch(
        `${API_BASE_URL}/getFarmConditions/${farmId}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
  ...
  async function fetchSensorData() {
    try {
      ...
      // }
      const response = await fetch(
        `${API_BASE_URL}/sensors/status?farm_id=${farmId}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
      ...
      updateSensorUI("co2", data.co2, "co2", 0, 800);
    } catch (error) {
  ...
  function updateSensorUI(sensorType, value, className, min, max) {
    ...
    const progressBar = document.querySelector(
      `.progress-container .progress-bar.${className}`
    );
    if (progressBar) {
  ...
  async function fetchDevicesStatus() {
    try {
      const response = await fetch(
        `${API_BASE_URL}/devices/status?farm_id=${farmId}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
  ...
  async function fetchRealtimeData() {
    try {
      const response = await fetch(
        `${API_BASE_URL}/realtime-data?farm_id=${farmId}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
  ...
  async function updateChartData() {
    ...
    if (!window.realtimeChart) {
        ...
        options: {
          ...
          scales: {
            y1: {
              beginAtZero: false,
              position: "left",
              ticks: { max: 80, min: 0, color: "#000000" },
            },
            y2: {
              beginAtZero: false,
              position: "right",
      ...
      window.realtimeChart.data.datasets[0].data = realtimeData.map(
        (item) => item.temperature
      );
      window.realtimeChart.data.datasets[1].data = realtimeData.map(
        (item) => item.humidity
      );
      window.realtimeChart.data.datasets[2].data = realtimeData.map(
        (item) => item.soil
      );
      window.realtimeChart.data.datasets[3].data = realtimeData.map(
        (item) => item.co2
      );
  ...
  async function fetchHistoryData() {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/history-data?farm_id=${farmId}&date=${formattedDate}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
      ...
      const processedData = {
        timeLabels: data.map((item) =>
          new Date(item.time_interval).toLocaleString("en-GB", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          })
        ...
        temperatureData: data.map((item) =>
          Number.parseFloat(item.avg_temperature)
        ),
  ...
  async function updateAllCharts() {
    ...
    const tempAvg =
      historyData.temperatureData.reduce((a, b) => a + b, 0) /
      historyData.temperatureData.length;
    const humidityAvg =
      historyData.humidityData.reduce((a, b) => a + b, 0) /
      historyData.humidityData.length;
    const soilAvg =
      historyData.soilData.reduce((a, b) => a + b, 0) /
      historyData.soilData.length;
    const co2Avg =
      historyData.co2Data.reduce((a, b) => a + b, 0) /
      historyData.co2Data.length /
      10;
  ...
  function formatDateTime(dateString) {
    ...
    return `${kstDate.getFullYear()}년 ${
      kstDate.getMonth() + 1
    }월 ${kstDate.getDate()}일 ${kstDate.getHours()}시 ${kstDate.getMinutes()}분`;
  }

  async function fetchAlarm() {
    try {
      const response = await fetch(
        `${API_BASE_URL}/getAlarm?farm_id=${farmId}`
      );
      ...
      const latestWarning = allAlarms.find(
        (alarm) => alarm.type === "경고"
      ) || {
        content: "알림 없음",
        created_at: "시간",
      };
      const latestComplete = allAlarms.find(
        (alarm) => alarm.type === "완료"
      ) || {
        content: "알림 없음",
        created_at: "시간",
  ...
  function fetchAlarmList() {
    const alarmListTableBody = document.querySelector(
      "#alarm-list-table tbody"
    );
    ...
    if (allAlarms.length === 0) {
      ...
      const sortedAlarms = allAlarms.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
      const filteredAlarms = sortedAlarms.filter(
        (alarm) => !selectedType || alarm.type === selectedType
      );
  ...
  async function generateReport() {
    try {
      ...
      let reportGenContainer = document.getElementById(
      ...
      const statusEl = reportGenContainer.querySelector(
        ".report-generation-text"
      );
      statusEl.textContent = "리포트 작성 중...";

      // 타이핑 애니메이션을 위한 컨텐츠 컨테이너 가져오기
      const contentEl = reportGenContainer.querySelector(
      ...
      // 오류 발생 시 생성 컨테이너 숨기기
      const reportGenContainer = document.getElementById(
        "reportGenerationContainer"
      );
      if (reportGenContainer) {
        reportGenContainer.style.display = "none";
  ...
  async function fetchReports() {
    try {
      ...
      if (reports.length === 0) {
        diaryEntries.innerHTML =
          '<li class="no-reports">생성된 리포트가 없습니다.</li>';
      } else {
        // 날짜 기준으로 정렬 (최신순)
        ...
        sortedReports.forEach((report) => {
          ...
          const formattedDate = `${reportDate.getFullYear()}년 ${
            reportDate.getMonth() + 1
          }월 ${reportDate.getDate()}일`;
  ...
  async function fetchLatestPlantImage() {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/latest-image?farmId=${farmId}`
      );
  ...
  function showReportModal(report) {
    ...
    if (avgTempEl)
      avgTempEl.textContent = `${report.sensorSummary.avg_temperature} °C`;
    if (avgHumidityEl)
      avgHumidityEl.textContent = `${report.sensorSummary.avg_humidity} %`;
    if (avgSoilEl)
      avgSoilEl.textContent = `${report.sensorSummary.avg_soil_moisture} %`;
    ...
    if (maxTempEl)
      maxTempEl.textContent = `${report.sensorChanges.max_temperature.value} °C`;
    if (maxTempTimeEl)
      maxTempTimeEl.textContent = report.sensorChanges.max_temperature.time;
    if (minTempEl)
      minTempEl.textContent = `${report.sensorChanges.min_temperature.value} °C`;
    if (minTempTimeEl)
      minTempTimeEl.textContent = report.sensorChanges.min_temperature.time;
    ...
    if (maxHumidityEl)
      maxHumidityEl.textContent = `${report.sensorChanges.max_humidity.value} %`;
    if (maxHumidityTimeEl)
      maxHumidityTimeEl.textContent = report.sensorChanges.max_humidity.time;
    if (minHumidityEl)
      minHumidityEl.textContent = `${report.sensorChanges.min_humidity.value} %`;
    if (minHumidityTimeEl)
      minHumidityTimeEl.textContent = report.sensorChanges.min_humidity.time;
    ...
    if (maxSoilEl)
      maxSoilEl.textContent = `${report.sensorChanges.max_soil_moisture.value} %`;
    if (maxSoilTimeEl)
      maxSoilTimeEl.textContent = report.sensorChanges.max_soil_moisture.time;
    if (minSoilEl)
      minSoilEl.textContent = `${report.sensorChanges.min_soil_moisture.value} %`;
    if (minSoilTimeEl)
      minSoilTimeEl.textContent = report.sensorChanges.min_soil_moisture.time;
    ...
    if (maxCo2El)
      maxCo2El.textContent = `${report.sensorChanges.max_co2.value} ppm`;
    if (maxCo2TimeEl)
      maxCo2TimeEl.textContent = report.sensorChanges.max_co2.time;
    if (minCo2El)
      minCo2El.textContent = `${report.sensorChanges.min_co2.value} ppm`;
    if (minCo2TimeEl)
      minCo2TimeEl.textContent = report.sensorChanges.min_co2.time;
    ...
    if (downloadReportBtn) {
      ...
      downloadReportBtn.parentNode.replaceChild(
        newDownloadBtn,
        downloadReportBtn
      );
...
2. 센서 수치 변화
최고 온도: ${report.sensorChanges.max_temperature.value} °C (시간: ${
      report.sensorChanges.max_temperature.time
    })
최저 온도: ${report.sensorChanges.min_temperature.value} °C (시간: ${
      report.sensorChanges.min_temperature.time
    })
최고 습도: ${report.sensorChanges.max_humidity.value} % (시간: ${
      report.sensorChanges.max_humidity.time
    })
최저 습도: ${report.sensorChanges.min_humidity.value} % (시간: ${
      report.sensorChanges.min_humidity.time
    })
최고 토양 수분: ${report.sensorChanges.max_soil_moisture.value} % (시간: ${
      report.sensorChanges.max_soil_moisture.time
    })
최저 토양 수분: ${report.sensorChanges.min_soil_moisture.value} % (시간: ${
      report.sensorChanges.min_soil_moisture.time
    })
최고 CO₂ 농도: ${report.sensorChanges.max_co2.value} ppm (시간: ${
      report.sensorChanges.max_co2.time
    })
최저 CO₂ 농도: ${report.sensorChanges.min_co2.value} ppm (시간: ${
      report.sensorChanges.min_co2.time
    })

3. 제어 장치 작동 기록
LED: ${
      report.deviceLogs.led.start
        ? `켜짐(시작: ${report.deviceLogs.led.start}, 종료: ${report.deviceLogs.led.end})`
        : "꺼짐"
    }
환기팬: 작동 횟수 ${report.deviceLogs.fan.count}회, 총 작동 시간 ${
      report.deviceLogs.fan.total_time
    }분
급수장치: 급수 횟수 ${report.deviceLogs.water.count}회, 총 급수량 ${
      report.deviceLogs.water.total_amount
    } L
히터: 작동 횟수 ${report.deviceLogs.heater.count}회, 총 작동 시간 ${
      report.deviceLogs.heater.total_time
    }분
쿨러: 작동 횟수 ${report.deviceLogs.cooler.count}회, 총 작동 시간 ${
      report.deviceLogs.cooler.total_time
    }분
