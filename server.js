...
app.get("/latest-image", async (req, res) => {
  ...
  if (!farmId) {
    return res
      .status(400)
      .json({ error: "farmId 쿼리 파라미터가 필요합니다." });
  }
  ...
  try {
    ...
    if (files.length === 0) {
      return res
        .status(404)
        .json({ error: "이 농장에 저장된 이미지가 없습니다." });
    }
...
app.post("/addFarm", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // 1. 농장 삽입
    const farmResult = await conn.query(insertFarmQuery, [
      user_id,
      farm_name,
      farm_location,
      farm_type,
    ...
    console.log(
      `[POST /addFarm] 농장 추가 성공: user_id=${user_id}, farm_id=${farm_id}`
    );
    ...
    const cropConditions = await conn.query(selectCropConditionsQuery, [
      farm_type,
    ]);
...
app.post("/delFarm", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (farmResults.affectedRows === 0) {
      return res
        .status(400)
        .json({ message: "해당 농장이 DB에 존재하지 않습니다." });
    }
...
app.post("/sensors", async (req, res) => {
  const { farm_id, temperature, humidity, soil_moisture, co2, created_at } =
    req.body;
...
app.get("/sensors/status", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "해당 조건에 맞는 데이터가 없습니다." });
    }
    console.log(`[GET /sensors/status] farm_id=${farm_id} 센서 조회`);
...
app.post("/devices/:deviceId/status", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[/devices/:deviceId/status] 제어장치 변경 성공: ${device} -> ${status}`
    );
    ...
    if (status == 1) {
      ...
      console.log(
        `[/devices/:deviceId/status] ${device} 작동 횟수 증가: farm_id=${farm_id}, date=${today}`
      );
    }
...
app.post("/devices/force-status", async (req, res) => {
  ...
  if (!farm_id || !device || duration == null) {
    return res
      ...
      .json({
        message:
          "잘못된 요청입니다. 모든 필드(farm_id, device, status, duration)가 필요합니다.",
      });
  }
  ...
  try {
    ...
    console.log(
      `[/devices/force-status] ${farm_id}농장 ${device} 상태 ${status}로 ${duration}시간 만큼 변경`
    );
    ...
    console.log(
      "[/devices/force-status] H/W 서버에 상태 및 지속시간 전달 성공"
    );
...
//     // 로그 출력
//     console.log(`[GET /realtime-data] farm_id=${farm_id}, 실시간 데이터: ${results.length}개`);
...
app.get("/history-data", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "해당 날짜에 기록된 데이터가 없습니다." });
    }

    console.log(
      `[GET /history-data] farm_id=${farm_id} 기록 데이터: ${results.length}개`
    );
    res.json(results);
  } catch (err) {
...
app.post("/start-farm", async (req, res) => {
  ...
  try {
    ...
    // 1. farms 테이블 업데이트
    const updateResult = await conn.query(updateFarmQuery, [
      currentDate,
      farmId,
    ]);
...
app.get("/get-farm-status/:farmId", async (req, res) => {
  ...
  try {
    ...
    const { farm_name, growth_rate, harvest_days, start_date, farm_active } =
      results[0];
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "농장 정보에 누락된 값이 있습니다." });
    }
...
app.get("/getFarmConditions/:farm_id", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: `${farm_id}농장에 대한 데이터가 없습니다` });
    }
  ...
  try {
    ...
    for (const [type, [min, max]] of Object.entries(values)) {
      ...
      if (result.affectedRows === 0) {
        console.warn(
          `[POST /updateFarmCondition] 데이터 없음: ${farm_id} - ${type} (업데이트 안 됨)`
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[POST /updateFarmCondition] 하드웨어 서버로 ${farm_id} 농장 최적 수치 전송 완료`
      );
    } catch (axiosError) {
      console.error(
        "[POST /updateFarmCondition] 하드웨어 서버 전송 오류:",
        axiosError.message
      );
    ...
    console.log(
      `[POST /updateFarmCondition] ${farm_id}농장 최적 수치 업데이트 완료`
    );
    return res.json({
      message: `${farm_id}농장의 최적 수치가 성공적으로 업데이트되었습니다`,
    });
  } catch (err) {
...
app.post("/generate-report", async (req, res) => {
  ...
  try {
    ...
    if (!datePattern.test(date)) {
      return res
        .status(400)
        .json({ error: "유효한 날짜 형식이 아닙니다 (YYYY-MM-DD)" });
    }
    ...
    let existingReport = Array.isArray(queryResult)
      ? queryResult
      : queryResult?.rows || [];
    if (existingReport.length > 0) {
      return res
        .status(409)
        .json({ error: "해당 날짜의 리포트가 이미 존재합니다." });
    }
      ...
      if (farmResult.length > 0) {
        ...
        if (farmResult[0].start_date && farmResult[0].harvest_days) {
          growthStage = getGrowthStage(
            farmResult[0].start_date,
            farmResult[0].harvest_days
          );
        }
      } else {
    ...
    if (
      ...
      !historyData.timeLabels.length
    ) {
      return res
        .status(400)
        .json({ error: "해당 날짜의 센서 데이터가 부족합니다" });
    }
    ...
    const sensorChanges = {
      max_temperature: {
        value: extremes.temperature.max,
        time: formatDateTime(extremes.temperature.maxTime),
      },
      min_temperature: {
        value: extremes.temperature.min,
        time: formatDateTime(extremes.temperature.minTime),
      },
      max_humidity: {
        value: extremes.humidity.max,
        time: formatDateTime(extremes.humidity.maxTime),
      },
      min_humidity: {
        value: extremes.humidity.min,
        time: formatDateTime(extremes.humidity.minTime),
      },
      max_soil_moisture: {
        value: extremes.soilMoisture.max,
        time: formatDateTime(extremes.soilMoisture.maxTime),
      },
      min_soil_moisture: {
        value: extremes.soilMoisture.min,
        time: formatDateTime(extremes.soilMoisture.minTime),
      },
      max_co2: {
        value: extremes.co2.max,
        time: formatDateTime(extremes.co2.maxTime),
      },
      min_co2: {
        value: extremes.co2.min,
        time: formatDateTime(extremes.co2.minTime),
      },
    };
    ...
    if (
      sensorChanges.max_temperature.value >
        optimalConditions.temperature?.optimal_max * 1.2 &&
      deviceLogs.fan.count === 0
    ) {
      criticalIssues.push(
        `고온 (${sensorChanges.max_temperature.value}℃)에도 환기팬 작동 없음`
      );
    }
    if (
      sensorChanges.max_humidity.value >
        optimalConditions.humidity?.optimal_max * 1.2 &&
      deviceLogs.fan.count === 0
    ) {
      criticalIssues.push(
        `고습 (${sensorChanges.max_humidity.value}%)에도 환기팬 작동 없음`
      );
    }
    if (
      sensorChanges.max_co2.value >
      optimalConditions.co2?.optimal_max * 1.5
    ) {
    ...
    const prompt = `
      당신은 스마트팜 데이터 분석 전문가입니다. 아래 데이터를 기반으로, 지정된 작물(${
        req.body.cropType || "일반 작물"
      })의 생육 조건과 성장 단계(${growthStage.stage}, 진행률 ${
      growthStage.progress
    }%)를 고려하여 간결하고 명확한 일일 리포트 요약을 작성하세요. 데이터 신뢰성, 이상 징후, 작물 건강에 대한 통찰, 실용적인 개선 제안을 포함하고, 각 항목을 한 줄로 간소화하세요.
      ...
      - 최적 온도: ${optimalConditions.temperature?.optimal_min || 20}-${
      optimalConditions.temperature?.optimal_max || 25
    }℃
      - 최적 습도: ${optimalConditions.humidity?.optimal_min || 60}-${
      optimalConditions.humidity?.optimal_max || 80
    }%
      - 최적 토양 수분: ${optimalConditions.soil_moisture?.optimal_min || 50}-${
      optimalConditions.soil_moisture?.optimal_max || 70
    }%
      - 최적 CO₂: ${optimalConditions.co2?.optimal_min || 400}-${
      optimalConditions.co2?.optimal_max || 1000
    }ppm

      6. 데이터 신뢰도:
      - 온도 데이터 누락: ${(
        (historyData.temperatureData.filter((v) => v === 0 || v === null)
          .length /
          historyData.temperatureData.length) *
        100
      ).toFixed(1)}%
      - 습도 데이터 누락: ${(
        (historyData.humidityData.filter((v) => v === 0 || v === null).length /
          historyData.humidityData.length) *
        100
      ).toFixed(1)}%
      - 토양 수분 데이터 누락: ${(
        (historyData.soilData.filter((v) => v === 0 || v === null).length /
          historyData.soilData.length) *
        100
      ).toFixed(1)}%
      - CO₂ 데이터 누락: ${(
        (historyData.co2Data.filter((v) => v === 0 || v === null).length /
      ...
      messages: [
        {
          role: "system",
          content:
            "당신은 스마트팜 데이터 분석 전문가입니다. 지정된 형식을 정확히 따르고, 간결하고 명확하게 요약하세요.",
        },
    ...
    if (
      ...
      !lines.every((line, i) => line.startsWith(expectedLines[i]))
    ) {
      console.warn("AI 응답 형식이 예상과 다릅니다. 보정합니다.");
      aiAnalysis = expectedLines
        .map((prefix, i) => {
          const matchingLine =
...
📊 센서 변화
- 최고 온도: ${sensorChanges.max_temperature.value}℃ (${
      sensorChanges.max_temperature.time
    })
- 최저 온도: ${sensorChanges.min_temperature.value}℃ (${
      sensorChanges.min_temperature.time
    })
- 최고 습도: ${sensorChanges.max_humidity.value}% (${
      sensorChanges.max_humidity.time
    })
- 최저 습도: ${sensorChanges.min_humidity.value}% (${
      sensorChanges.min_humidity.time
    })
- 최고 토양 수분: ${sensorChanges.max_soil_moisture.value}% (${
      sensorChanges.max_soil_moisture.time
    })
- 최저 토양 수분: ${sensorChanges.min_soil_moisture.value}% (${
      sensorChanges.min_soil_moisture.time
    })
...
⚙️ 제어 장치
- LED: ${
      deviceLogs.led.start
        ? `켜짐 (${deviceLogs.led.start}~${deviceLogs.led.end})`
        : "꺼짐"
    }
...
async function fetchHistoryDataFromDB(farmId, date) {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/history-data?farm_id=${farmId}&date=${date}`,
      {
        method: "GET",
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || "센서 데이터를 불러오는데 실패했습니다"
      );
    }
    ...
    const result = {
      timeLabels: historyData.map((row) =>
        new Date(row.time_interval).toISOString().slice(11, 16)
      ),
      temperatureData: historyData.map(
        (row) => Number(row.avg_temperature) || 0
      ),
    ...
    console.error(
      `센서 데이터 조회 실패 - 농장 ID: ${farmId}, 날짜: ${date}`,
      error
    );
...
async function fetchExtremeSensorDataFromDB(farmId, date) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || "센서 극값 데이터를 불러오는데 실패했습니다"
      );
    }
    ...
    console.error(
      `센서 극값 데이터 조회 실패 - 농장 ID: ${farmId}, 날짜: ${date}`,
      error
    );
...
async function fetchDeviceLogs(farmId, date) {
  ...
  try {
    ...
    console.error(
      `[fetchDeviceLogs] 오류: farmId=${farmId}, date=${date}`,
      error
    );
...
app.get("/get-reports/:farmId", async (req, res) => {
  ...
  try {
    ...
    const formattedReports = reports.map((report) => {
      ...
      const sensorSummary =
        typeof report.sensor_summary === "string"
          ? JSON.parse(report.sensor_summary)
          : report.sensor_summary;
      const sensorChanges =
        typeof report.sensor_changes === "string"
          ? JSON.parse(report.sensor_changes)
          : report.sensor_changes;
      const deviceLogs =
        typeof report.device_logs === "string"
          ? JSON.parse(report.device_logs)
          : report.device_logs;
